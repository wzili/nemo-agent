      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-left pt-2">
            {t('dialog.resetAppConfirm')}
          </DialogDescription>
        </DialogHeader>

        <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1 pl-2">
          <li>{t('dialog.workspacesAndSettings')}</li>
          <li>{t('dialog.credentialsAndKeys')}</li>
          <li>{t('dialog.preferencesAndData')}</li>
        </ul>

        <div className="bg-amber-500/10 border border-amber-500/30 rounded-md p-3 text-sm">
          <strong className="text-amber-600 dark:text-amber-400">{t('dialog.resetAppWarning')}</strong>
        ...
        <div className="space-y-2 pt-2">
          <label className="text-sm font-medium">
            {t('dialog.resetAppSolve')} {problem.a} + {problem.b} =
          </label>
          <Input
            type="text"
            inputMode="numeric"
            pattern="[0-9]*"
            placeholder={t('dialog.resetAppEnterAnswer')}
            value={answer}
