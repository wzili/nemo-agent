...
export function SessionMenu({
  ...
  const menuLabels = buildMenuLabels(t, getFileManagerName())
  ...
  const handleShare = async () => {
    ...
    if (result?.success && result.url) {
      ...
      toast.success(menuLabels.session.linkCopied, {
        description: result.url,
        action: {
          label: menuLabels.session.open,
          onClick: () => window.electronAPI.openUrl(result.url!),
        },
      })
    } else {
      toast.error(menuLabels.session.failedToShare, { description: result?.error || menuLabels.session.unknownError })
    }
  }
  ...
  const handleCopyPath = async () => {
    ...
    if (result?.success && result.path) {
      ...
      toast.success(menuLabels.session.pathCopied)
    }
  }
  ...
  const handleRefreshTitle = async () => {
    ...
    if (result?.success) {
      toast.success(menuLabels.session.titleRefreshed, { description: result.title })
    } else {
      toast.error(menuLabels.session.failedToRefreshTitle, { description: result?.error || menuLabels.session.unknownError })
    }
  }
    ...
    <>
      ...
      {!sharedUrl ? (
        <MenuItem onClick={handleShare}>
          ...
          <span className="flex-1">{menuLabels.session.share}</span>
        </MenuItem>
      ) : (
        <Sub>
          <SubTrigger className="pr-2">
            <CloudUpload className="h-3.5 w-3.5" />
            <span className="flex-1">{menuLabels.session.shared}</span>
          </SubTrigger>
          <SubContent>
      ...
      <Sub>
        <SubTrigger className="pr-2">
          ...
          <span className="flex-1">{menuLabels.session.status}</span>
        </SubTrigger>
        <SubContent>
          <StatusMenuItems
      ...
      {labels.length > 0 && (
        <Sub>
          <SubTrigger className="pr-2">
            ...
            <span className="flex-1">{menuLabels.session.labels}</span>
      ...
      {!isFlagged ? (
        <MenuItem onClick={onFlag}>
          ...
          <span className="flex-1">{menuLabels.session.flag}</span>
        </MenuItem>
      ) : (
        <MenuItem onClick={onUnflag}>
          <FlagOff className="h-3.5 w-3.5" />
          <span className="flex-1">{menuLabels.session.unflag}</span>
        </MenuItem>
      )}
      ...
      {!isArchived ? (
        <MenuItem onClick={onArchive}>
          ...
          <span className="flex-1">{menuLabels.session.archive}</span>
        </MenuItem>
      ) : (
        <MenuItem onClick={onUnarchive}>
          <ArchiveRestore className="h-3.5 w-3.5" />
          <span className="flex-1">{menuLabels.session.unarchive}</span>
        </MenuItem>
      )}
      ...
      {!_hasUnread && _hasMessages && (
        <MenuItem onClick={onMarkUnread}>
          ...
          <span className="flex-1">{menuLabels.session.markAsUnread}</span>
        </MenuItem>
      )}
      ...
      <MenuItem onClick={onRename}>
        ...
        <span className="flex-1">{menuLabels.session.rename}</span>
      </MenuItem>

      {/* Regenerate Title - AI-generate based on recent messages */}
      <MenuItem onClick={handleRefreshTitle}>
        <RefreshCw className="h-3.5 w-3.5" />
        <span className="flex-1">{menuLabels.session.regenerateTitle}</span>
      </MenuItem>
      ...
      <MenuItem onClick={handleOpenInNewPanel}>
        ...
        <span className="flex-1">{menuLabels.session.openInNewPanel}</span>
      </MenuItem>

      {/* Open in New Window */}
      <MenuItem onClick={onOpenInNewWindow}>
        <AppWindow className="h-3.5 w-3.5" />
        <span className="flex-1">{menuLabels.session.openInNewWindow}</span>
      </MenuItem>

      {/* Show in file manager */}
      <MenuItem onClick={handleShowInFinder}>
        <FolderOpen className="h-3.5 w-3.5" />
        <span className="flex-1">{menuLabels.session.showInApp}</span>
      </MenuItem>

      {/* Copy Path */}
      <MenuItem onClick={handleCopyPath}>
        <Copy className="h-3.5 w-3.5" />
        <span className="flex-1">{menuLabels.session.copyPath}</span>
      </MenuItem>
      ...
      <MenuItem onClick={onDelete} variant="destructive">
        ...
        <span className="flex-1">{menuLabels.session.delete}</span>
      </MenuItem>
    </>
  )
}
