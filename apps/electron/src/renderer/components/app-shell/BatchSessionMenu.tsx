...
export function BatchSessionMenu() {
  ...
  const handleBatchFlag = useCallback(() => {
    ...
    toast(t('session.sessionsFlagged', { count: selectedIds.size }))
  }, [selectedIds, onFlagSession, t])

  const handleBatchUnflag = useCallback(() => {
    selectedIds.forEach(id => onUnflagSession(id))
    toast(t('session.sessionsUnflagged', { count: selectedIds.size }))
  ...
  const handleBatchArchive = useCallback(() => {
    ...
    clearMultiSelect()
    toast(t('session.sessionsArchived', { count: selectedIds.size }))
  ...
  const handleBatchDelete = useCallback(async () => {
    ...
    clearMultiSelect()
    toast(t('session.sessionsDeleted', { count }))
  ...
  return (
    <>
      ...
      <div className="px-2 py-1.5 text-xs text-muted-foreground font-medium">
        {t('session.sessionsSelected', { count })}
      </div>
      <Separator />
      ...
      <Sub>
        <SubTrigger className="pr-2">
          ...
          <span className="flex-1">{t('session.status')}</span>
        </SubTrigger>
        <SubContent>
          <StatusMenuItems
      ...
      {labels.length > 0 && (
        <Sub>
          <SubTrigger className="pr-2">
            ...
            <span className="flex-1">{t('session.labels')}</span>
          </SubTrigger>
          <SubContent>
            <LabelMenuItems
              labels={labels}
      ...
      {allFlagged ? (
        <MenuItem onClick={handleBatchUnflag}>
          ...
          <span className="flex-1">{t('session.unflagAll')}</span>
        </MenuItem>
      ) : (
        <MenuItem onClick={handleBatchFlag}>
          <Flag className="h-3.5 w-3.5 text-info" />
          <span className="flex-1">{t('session.flagAll')}</span>
        </MenuItem>
      )}
      ...
      <MenuItem onClick={handleBatchArchive}>
        ...
        <span className="flex-1">{t('session.archive')}</span>
      </MenuItem>
      ...
      <MenuItem onClick={handleBatchDelete} variant="destructive">
        ...
        <span className="flex-1">{t('session.delete')}</span>
      </MenuItem>
    </>
  )
}
