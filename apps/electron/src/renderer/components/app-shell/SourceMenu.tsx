  ...
  return (
    <>
      ...
      <MenuItem onClick={onOpenInNewWindow}>
        ...
        <span className="flex-1">{labels.source.openInNewWindow}</span>
      </MenuItem>

      {/* Show in file manager */}
      <MenuItem onClick={onShowInFinder}>
        <FolderOpen className="h-3.5 w-3.5" />
        <span className="flex-1">{labels.source.showInApp}</span>
      </MenuItem>
      ...
      <MenuItem onClick={onDelete} variant="destructive">
        ...
        <span className="flex-1">{labels.source.delete}</span>
      </MenuItem>
    </>
  )
}
