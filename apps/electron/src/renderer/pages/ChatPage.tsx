...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
        ...
        {sharedUrl ? (
          ...
          <>
            <StyledDropdownMenuItem onClick={handleShare}>
              ...
              <span className="flex-1">{t('menu.shareOnline')}</span>
            </StyledDropdownMenuItem>
            <StyledDropdownMenuSeparator />
            <StyledDropdownMenuItem onClick={() => window.electronAPI.openUrl('https://agents.craft.do/docs/go-further/sharing')}>
              <Info className="h-3.5 w-3.5" />
  ...
  if (!session) {
    ...
    return (
      <div className="h-full flex flex-col">
        <PanelHeader  title={t('session.session')} rightSidebarButton={rightSidebarButton} />
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground">
          <AlertCircle className="h-10 w-10" />
          <p className="text-sm">{t('session.sessionNoLongerExists')}</p>
        </div>
      </div>
    )
  }
